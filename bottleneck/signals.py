# -*- coding: utf-8 -*-
"""
signals.py
Stage 1 룰 필터의 핵심. 형님 키워드 분류 체계로 재구성.
'호재 뉴스'가 아니라 '구조적 수급/가격/경쟁/CAPEX/정책 신호'만 잡는다.

설계 원칙:
- Top 20 핵심 단어는 가중치를 높여(weight 4) S급 신호로 취급.
- 일반 카테고리 단어는 weight 2~3.
- 여러 축(수급+가격+경쟁 등)에 동시 매칭될수록 점수가 급등 → S급만 살아남음.
- 키워드 추가/삭제는 이 파일에서만.

[중복 제거 적용 - 2026.07.01]
- capex/CAPEX 완전 중복 제거 (소문자 capex만 유지)
- ASP 상승(pricing) 삭제: core_top의 'ASP'가 이미 포함하여 이중 카운트
- 독과점(competition) 삭제: core_top의 '과점'이 이미 포함
- 캐파 증설(capex) 삭제: core_top의 '증설'이 이미 포함
- 공급망(supply) 삭제: 범용 단어라 노이즈 유발 + '공급망 차질'과 중복
- 표준(competition) 삭제: core_top의 '표준화'가 이미 포함
- 수주/수주잔고는 의도적 가중치 차등으로 판단, 유지
"""

# (키워드들, 가중치)
SIGNAL_CATEGORIES = {
    # ===== S급 핵심 20 (가중치 최상) =====
    "core_top": (
        [
            "쇼티지", "shortage", "병목", "bottleneck", "공급 부족",
            "수주잔고", "백로그", "backlog", "주문 급증", "가격 인상",
            "ASP", "점유율 확대", "독점", "과점", "증설",
            "capex", "재고 소진", "슈퍼사이클", "supercycle",
            "구조적 성장", "표준화", "리드타임", "lead time",
            "락인", "lock-in", "규제 수혜", "TAM",
        ],
        4,
    ),
    # ===== 수급 =====
    "supply": (
        [
            "공급 제약", "공급 차질", "재고 부족", "품귀", "슬롯 부족",
            "생산능력 부족", "캐파 부족", "공급망 차질",
            "출하 지연", "납기 지연", "공급 타이트", "tight supply",
        ],
        3,
    ),
    # ===== 수요 =====
    "demand": (
        [
            "수요 증가", "수요 급증", "수주", "발주", "예약 증가",
            "고객 확대", "채택률", "침투율", "adoption", "신규 고객",
        ],
        3,
    ),
    # ===== 가격 =====
    "pricing": (
        [
            "가격 협상", "협상력", "프리미엄", "믹스 개선",
            "단가 인상", "가격 급등", "spot price", "현물가", "계약가",
            "고정거래가", "renegotiation", "재협상",
        ],
        3,
    ),
    # ===== 산업 사이클 =====
    "cycle": (
        [
            "업사이클", "재고 정상화", "공급 축소", "감산", "산업 재편",
            "구조조정", "사이클 반등", "턴어라운드",
        ],
        2,
    ),
    # ===== 경쟁 구도 =====
    "competition": (
        [
            "진입장벽", "기술 격차", "기술 초격차", "standard",
            "시장 지배", "지배력",
        ],
        3,
    ),
    # ===== CAPEX / 투자 =====
    "capex": (
        [
            "투자 확대", "신규 공장", "생산능력 확장",
            "인프라 투자", "데이터센터 건설", "데이터센터 투자",
            "설비 투자", "가이던스", "guidance", "hyperscaler",
        ],
        3,
    ),
    # ===== 정책 / 규제 =====
    "policy": (
        [
            "규제 완화", "보조금", "세액공제", "국산화", "리쇼어링",
            "reshoring", "전략산업", "수출 통제", "export control",
            "수출 제한", "관세", "tariff",
        ],
        3,
    ),
    # ===== 쇼티지·원자재 (원자재 급등락) =====
    "material": (
        [
            "원자재", "희토류", "rare earth", "구리", "copper", "리튬",
            "니켈", "코발트", "갈륨", "게르마늄", "네온", "팔라듐",
            "우라늄", "uranium", "유가", "원유", "천연가스", "곡물",
        ],
        3,
    ),
    # ===== 지정학 / 전쟁 / 방산 =====
    "geopolitics": (
        [
            "전쟁", "war", "교전", "공습", "미사일", "확전", "휴전",
            "지정학", "geopolitical", "제재", "sanction", "봉쇄", "blockade",
            "호르무즈", "Hormuz", "대만", "Taiwan", "남중국해", "분쟁",
            "방산", "defense", "무기", "군비", "국방 예산",
        ],
        3,
    ),
}

# ===== 임계값 =====
# 최초 9에서 1만 낮춘 8. (처음이 너무 빡셌고, 5/7은 너무 풀렸음)
# 더 풀려면 7, 더 조이려면 9.
SCORE_THRESHOLD = 8

# 하루 최종 전송 상한
MAX_DAILY = 5

CATEGORY_LABELS = {
    "core_top": "핵심신호",
    "supply": "수급",
    "demand": "수요",
    "pricing": "가격",
    "cycle": "사이클",
    "competition": "경쟁",
    "capex": "CAPEX",
    "policy": "정책",
    "material": "원자재",
    "geopolitics": "지정학",
}
