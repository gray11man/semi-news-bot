...
class BudgetApiTests(unittest.TestCase):
    ...
    def test_real_payload_shape_and_usage_accounting_with_mock_http(self):
        ...
        count_payload=json.loads(call.call_args_list[0].args[0].data)
        self.assertIn('systemInstruction',count_payload['generateContentRequest'])
        self.assertEqual(count_payload['generateContentRequest']['model'],'models/gemini-3.8-flash')
        self.assertNotIn('generationConfig',count_payload['generateContentRequest'])
        generation_payload=json.loads(call.call_args_list[1].args[0].data)
        self.assertIn('responseFormat',generation_payload['generationConfig'])
        a.budget.db.close()

    def test_legacy_model_uses_legacy_structured_output_config(self):
        a=Gemini('x','gemini-2.5-flash')
        schema={'type':'object','properties':{'title':{'type':'string','maxLength':20}},
                'required':['title'],'additionalProperties':False}
        data={'candidates':[{'finishReason':'STOP','content':{'parts':[{'text':'{"title":"ok"}'}]}}],
              'usageMetadata':{'totalTokenCount':20}}
        with patch('network.urllib.request.urlopen',side_effect=[response({'totalTokens':10}),response(data)]) as call:
            self.assertEqual(a.json('system',{},schema),{'title':'ok'})
        payload=json.loads(call.call_args_list[1].args[0].data)
        ...
        self.assertEqual(config['responseSchema']['properties']['title']['type'],'STRING')
        self.assertNotIn('maxLength',config['responseSchema']['properties']['title'])
        self.assertNotIn('additionalProperties',config['responseSchema'])

    def test_http_error_detail_is_kept_short_and_actionable(self):
        error=urllib.error.HTTPError('https://example.test',400,'bad',{},
                                     io.BytesIO(b'{"error":{"message":"Invalid JSON payload received"}}'))
        self.assertEqual(Gemini._http_detail(error),'Invalid JSON payload received')
