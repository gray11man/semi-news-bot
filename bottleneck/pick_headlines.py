...
def _generation_config(model, review):
    max_tokens = 16384 if review else 8192
    ...
    if model.startswith("gemini-2.5"):
        budget = int(os.getenv("REVIEW_THINKING_BUDGET" if review else "PICK_THINKING_BUDGET", "4096" if review else "1024"))
...
def _pick(items, history, limit, review=False):
    ...
    records = [{
        "index": i,
        **{k: it.get(k, "") for k in (
            "title", "summary", "source", "published", "sector_hint", "sector", "feed_label"
        )},
...
def pick_critical(news_items, max_pick=None):
    ...
    try:
        ...
        batch_size = int(os.getenv("PICK_BATCH_SIZE", "55"))
        if not 20 <= batch_size <= 100:
            raise PickerError("PICK_BATCH_SIZE는 20~100")

        # 최종 6건을 뽑더라도 1차에서 각 배치 최대 8~10건을 살려 업종 다양성을 확보한다.
        candidate_limit = min(10, max(8, limit + 2))
        candidates = []
        successful_batches = 0
        failed_batches = 0
        review_items = _interleave_by_sector(news_items)

        for offset in range(0, len(review_items), batch_size):
            batch = review_items[offset:offset + batch_size]
            try:
                candidates.extend(_review_batch(batch, history, candidate_limit))
        ...
        # 최종 모델에는 여유 있게 최대 10건을 뽑게 한 뒤 코드에서 동일 섹터 과밀만 완화한다.
