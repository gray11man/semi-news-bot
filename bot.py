...
def tolerant_setting(name, default, lo, hi):
    """환경변수가 비어 있거나 숫자가 아니거나 범위를 벗어나도 실행을 중단하지 않는다."""
    raw = os.getenv(name)
    ...
    try:
        ...
        print(f'[WARN] {name}={raw!r} 숫자 아님 → 기본값 {default} 사용')
        return default
    if not lo <= value <= hi:
        print(f'[WARN] {name}={value} 허용범위 {lo}~{hi} 밖 → 기본값 {default} 사용')
        return default
    return value
...
def validate_review(rows, batch):
    ...
    for row in rows:
        ...
        if item.get('freshness_note') in strict_freshness:
            ...
            hours = tolerant_setting('NEWS_WINDOW_HOURS', 12, 1, 24)
...
def no_news_message(report, selected_count):
    ...
    window = tolerant_setting('NEWS_WINDOW_HOURS', 12, 1, 24)
