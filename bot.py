...
def validate_rows(rows, count, flag='keep', allow_partial=False):
    ...
    # 부분응답을 허용하는 단계에서는 Gemini가 드물게 중복 index나
    # 범위 밖 index를 붙여도 전체 실행을 죽이지 않고 유효 행만 사용한다.
    if allow_partial:
        cleaned = []
        indices = set()
        dropped = 0
        for row in rows:
            if not isinstance(row, dict):
                ...
                continue
            idx = row.get('index')
            if (type(idx) is not int or not 0 <= idx < count or
                    idx in indices or type(row.get(flag)) is not bool):
                dropped += 1
                continue
            indices.add(idx)
            cleaned.append(row)

        # 호출부가 반환값을 다시 대입하지 않는 곳도 있으므로 원본 리스트를 직접 정리한다.
        rows[:] = cleaned
        if dropped:
            print(f'[Gemini 응답 정리] 잘못된/중복 행 {dropped}건 제거')
        if len(rows) < count:
            print(f'[DEFER] {count}건 중 {len(rows)}건 유효 응답; 누락 {count-len(rows)}건은 대기 유지')
        return rows
...
def choose(state, api, checkpoint):
    ...
    ids = approved_ids[:40]
    batch = [pending[k] for k in ids]
    ranked = api.ask('최종 편집: 입력 articles에 실제로 존재하는 index만 중요도 순서로 반환한다. '
        '각 index는 정확히 한 번만 반환하고 중복 index나 범위 밖 index를 절대 만들지 않는다. '
        '출력 배열 길이는 입력 articles 개수를 절대 초과하지 않는다. '
        '동일 사건은 가장 근거가 충실한 한 건을 남기고 나머지를 duplicate=true로 표시한다. '
        '과거 sent와 동일 사건의 재탕도 true. 같은 기업의 다른 사건이나 중요한 새 업데이트는 중복이 아니다.',
