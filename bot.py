...
# -*- coding: utf-8 -*-
"""
AI·반도체·메모리·데이터센터·전력·AI수요 산업 뉴스 에이전트 (v2.8.1)

주요 이력:
  v2.7  과거 뉴스 3중 차단 (URL 날짜 / 원문 발행일 / Gemini C 판정),
        SEEN_RETENTION_DAYS 30일, STALE_HARD_LIMIT_H 48시간
  v2.7.2 네오클라우드(허트8·아이렌·코어위브 등)·임대계약 키워드 추가
  v2.7.3 구글 래퍼 페이지 날짜 오인 버그 수정 (전 기사 동일나이 오폐기 문제)
  v2.7.4 중국 AI 3개 언어 피드 + 소버린 AI 피드 + 일본 보강
  v2.7.5 Gemini 프롬프트에 투자 논제 판단기준 6개 삽입
  v2.7.6 메모리 가격지표·서버 ODM·냉각·벤더파이낸싱·SMR/PPA 보강,
        급등/급락 시황 오감점 버그 수정, 무효화 변형(착공 지연 등) 추가
  v2.7.7 오라클 리스크 전용 피드·CDS·RPO·등급 신호 (사슬의 최약 고리)
  v2.7.8 구글 뉴스 URL base64 디코딩으로 실제 기사 URL 확보,
        미검증+미분석 기사 전송 차단 (재색인 유입 최종 봉쇄)
  v2.7.9 MSN 재유통 플랫폼 원천 차단 (과거 기사 최대 오염원)
