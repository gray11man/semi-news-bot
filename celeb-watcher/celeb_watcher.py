...
- 통과한 것만 텔레그램 전송
- 네이버 블로그 6개 6시간 주기 감시 -> 텔레그램 전송
  (트위터 감시는 비용 문제로 비활성화)
- [NEW] 크레딧/사모대출 감시 모듈 연동 (credit_watch.py)
...
# [NEW] 크레딧 감시 모듈 (같은 폴더의 credit_watch.py)
...
NOTIFY_WHEN_EMPTY = False   # [NEW] True면 결과 없을 때도 "없음" 알림 전송
...
# ── 블로그 상태 로드/저장 (기존 seen과 분리된 별도 파일) ──
...
def main():
    ...
    if not candidates:
        ...
        for vid, item in candidates.items():
            ...
            try:
                ...
                # [FIX] Gemini 실패 시 seen에 넣지 않음 -> 다음 사이클 재시도
                print(f"Gemini 오류(다음 사이클 재시도): {e}")
                continue
            seen.add(vid)
            if ok:
    ...
    # 네이버 블로그 감시 (유튜브 파트와 독립적으로 실행)
    run_blog_watch()

    # [NEW] 크레딧/사모대출 감시 — 반드시 마지막 (셀럽봇이 Gemini 쿼터 우선 사용)
    run_credit_watch()
